...
# Updated function to look in multiple possible locations for the XYZ files
def format_xyz_for_si(filename, compound_id, substituent_name):
    # Check current directory first, then look inside alkyne_library
    possible_paths = [filename, os.path.join("alkyne_library", filename)]
    filepath = None
...
for comp, sub, fname in zip(compounds, substituents, files):
    formatted_block = format_xyz_for_si(fname, comp, sub)
...
def generate_milestone_5_pdf(dataframe, filename="Milestone_5_Report.pdf"):
    ...
    summary = (
        ...
        "Supporting Information (SI) tables. The Journal of Organic Chemistry maintains strict mandates for complete, "
        "reproducible raw coordinate lists to facilitate geometric replication."
    )
    ...
    doc.build(story)
    print(f"[SUCCESS] Updated Phase 5 Data Compiled.")
